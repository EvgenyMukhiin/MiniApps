...
namespace crc
{
    public partial class Form1 : Form
    {
        ...
        //{ 255,255,32,58,3 };  255,255,51,35,1
        private const long Crc24Init = 0x00b704ceL;//0x086885b;//0x0b704ce
        ...
        {
            var color = tb1.Text == "" ? tb1.BackColor = Color.Red : tb1.BackColor = Color.White;
            color = tb2.Text == "" ? tb2.BackColor = Color.Red : tb2.BackColor = Color.White;
            color = tb3.Text == "" ? tb3.BackColor = Color.Red : tb3.BackColor = Color.White;
            color = tb4.Text == "" ? tb4.BackColor = Color.Red : tb4.BackColor = Color.White;
            color = tb5.Text == "" ? tb5.BackColor = Color.Red : tb5.BackColor = Color.White;
            if (color.Name == "White")
            {
                byte[] octets = new byte[]
                {
                    b1 = byte.Parse(tb1.Text),
                    b2 = byte.Parse(tb2.Text),
                    b3 = byte.Parse(tb3.Text),
                    b4 = byte.Parse(tb4.Text),
                    b5 = byte.Parse(tb5.Text)
                };
            textBox1.Text = crc_octets(octets, octets.Length).ToString("X");
            textBox2.Text = crc_octets(octets, octets.Length).ToString();
            }
        }
        // Метод для расчета контрольной суммы
        static readonly long[] crcTab = new long[]
        {
        ...
        long crc_octets(byte[] octets, int len)
        {
            long crc = Crc24Init;
            long temp;
            int arg;
